...
export async function runPolymarketAutonomousTrading(
  ...
  const guardedSignals = reviewedSignals.filter((s) => {
    ...
    // round-trip fee math reflects a realistic Polymarket position
    // (~$5-20 USDC notional) rather than a $0.50 toy notional that would let
    // a flat 1-cent fee swamp the gross EV.  The actual order size is
    // determined later by the dynamic risk-limit sizer.
    const entry = Number((s as { limitPrice?: number; impliedProbabilityYes?: number }).limitPrice
      ?? (s as { impliedProbabilityYes?: number }).impliedProbabilityYes
      ?? 0.5);
