/**
 * Polymarket Signal Reviewer
 * 
 * Adapts the AI trader duo review (OpenAI + Claude) from Kalshi
 * to work with Polymarket signals. The reviewer acts as a conservative
 * gatekeeper that approves, vetoes, or adjusts signal confidence and EV.
 */
...
export function isPolymarketReviewerConfigured(
  ...
  return DEFAULT_DUO_PROVIDERS.every((provider) =>
...
function getReviewPayload(input: {
  ...
  const marketById = new Map(
    input.markets.map((market) => [market.marketId, market])
  );
...
function getAnthropicText(
  payload: Awaited<ReturnType<AnthropicClient["messages"]["create"]>>
) {
...
async function requestAnthropicReviews(
  ...
  const response = await Promise.race([
    ...
    new Promise<never>((_, reject) =>
      setTimeout(
        () => reject(new Error("Anthropic review timed out")),
        timeoutMs
      )
    ),
  ]);
...
function combineApprovedSignal(
  ...
  if (providerReviews.length === 0) {
    logger.warn(
      `[PolymarketReviewer] No provider approvals for marketId=${signal.marketId}; dropping signal.`
    );
    return null;
  }
  ...
  const confidenceAdjustment =
    providerReviews.reduce(
      (total, providerReview) =>
        total +
        clamp(Number(providerReview.review.confidenceAdjustment ?? 0), -0.25, 0.15),
      0
  ...
  const expectedValueAdjustment =
    providerReviews.reduce(
      (total, providerReview) =>
        total +
        clamp(Number(providerReview.review.expectedValueAdjustment ?? 0), -0.1, 0.1),
      0
    ) / providerReviews.length;
...
export async function reviewPolymarketSignalsWithTrader(
  ...
  const activeProviders = getActiveProviders(options);
  if (
    !DEFAULT_DUO_PROVIDERS.every((provider) => activeProviders.includes(provider))
  ) {
    logger.error(
      "[PolymarketReviewer] OpenAI + Claude duo is not fully configured; dropping all candidates so autonomous trading fails closed."
    );
    return [];
  }
  ...
  try {
    ...
    return input.signals
      .map((signal) => {
        const providerReviews = activeProviders.map((provider) => {
          ...
          if (!review) {
            logger.warn(
              `[PolymarketReviewer] ${provider} did not return a review for marketId=${signal.marketId}; dropping signal.`
            );
            return null;
          }
    ...
    logger.error(
      "[PolymarketReviewer] Duo AI review failed; dropping all candidates (no autonomous execution without explicit OpenAI + Claude approval):",
      error
    );
    return [];
  }
